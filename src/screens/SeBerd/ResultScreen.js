...
import { Trophy, RotateCcw, Home, Crown, Target, Swords } from 'lucide-react-native';
...
export default function SeBerdResultScreen({ navigation, route }) {
    const { winner, player1Name, player2Name, moveCount } = route.params;
    ...
    const handlePlayAgain = () => {
        ...
        navigation.navigate('SeBerdPlay', { player1Name, player2Name });
    };
    const handleNewGame = () => {
        if (Platform.OS !== 'web') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
        navigation.navigate('SeBerdSetup');
    };
    const handleGoHome = () => {
        if (Platform.OS !== 'web') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                ...
                <MotiView from={{ opacity: 0, translateY: 20 }} animate={{ opacity: 1, translateY: 0 }}
                    ...
                    <Text style={[styles.winLabel, { color: colors.accent }, isKurdish && styles.kf]}>
                        {isKurdish ? 'سێ بەرد لە ڕیزەکدا!' : 'Three in a Row!'}
                    </Text>
                ...
                <GlassCard style={{ marginTop: layout.spacing.lg, marginBottom: layout.spacing.xl }}>
                    <View style={[styles.statsRow, { flexDirection: rowDirection }]}>
                        <View style={styles.statItem}>
                            ...
                            <Text style={[styles.statValue, { color: colors.text.primary }]}>{moveCount || 0}</Text>
