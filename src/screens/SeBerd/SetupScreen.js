...
import { Play, Target, Users, Info } from 'lucide-react-native';
...
export default function SeBerdSetupScreen({ navigation }) {
    ...
    const handleStart = () => {
        ...
        navigation.navigate('SeBerdPlay', {
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
                ...
                <MotiView from={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
                    ...
                    <Text style={[styles.heroSub, { color: colors.text.muted }, isKurdish && styles.kf]}>
                        {isKurdish ? 'سێ بەرد لە ڕیزەکدا' : 'Three Stones in a Row'}
                    </Text>
                </MotiView>
                ...
                <GlassCard style={{ marginBottom: layout.spacing.lg }}>
                    <View style={[styles.infoRow, { flexDirection: rowDirection }]}>
                        ...
                        <View style={styles.infoItem}>
                            ...
                            <Text style={[styles.infoText, { color: colors.text.secondary }]}>3×3</Text>
                            <Text style={[styles.infoLabel, { color: colors.text.muted }, isKurdish && styles.kf]}>
                                {isKurdish ? 'تەختە' : 'Grid'}
                            </Text>
                        </View>
                        <View style={[styles.infoDivider, { backgroundColor: colors.border }]} />
                        <View style={styles.infoItem}>
                            <Text style={[styles.infoText, { color: colors.text.secondary }]}>3</Text>
                            <Text style={[styles.infoLabel, { color: colors.text.muted }, isKurdish && styles.kf]}>
                                {isKurdish ? 'بەرد' : 'Stones'}
                            </Text>
                        </View>
                    </View>
                </GlassCard>
                ...
                <GlassCard style={{ marginBottom: layout.spacing.md }}>
                    <View style={[styles.sectionHeader, { flexDirection: rowDirection }]}>
                        <View style={[styles.playerDot, { backgroundColor: '#3B82F6', borderColor: '#60A5FA' }]} />
                        <Text style={[styles.sectionTitle, { color: colors.text.secondary }, isKurdish && styles.kf]}>
                            {isKurdish ? 'یاریزانی یەکەم (شین)' : 'Player 1 (Blue)'}
                        </Text>
                    </View>
                    <PremiumInput
                        value={player1Name}
                ...
                <GlassCard style={{ marginBottom: layout.spacing.lg }}>
                    <View style={[styles.sectionHeader, { flexDirection: rowDirection }]}>
                        <View style={[styles.playerDot, { backgroundColor: '#EF4444', borderColor: '#F87171' }]} />
                        <Text style={[styles.sectionTitle, { color: colors.text.secondary }, isKurdish && styles.kf]}>
                            {isKurdish ? 'یاریزانی دووەم (سوور)' : 'Player 2 (Red)'}
                        </Text>
                    </View>
                    <PremiumInput
                        value={player2Name}
                ...
                <GlassCard>
                    ...
                    {[
                        isKurdish ? '١. هەر یاریزانێک ٣ بەردی هەیە' : '1. Each player has 3 stones',
                        isKurdish ? '٢. یەکەم: بەردەکان لە خانەی بەتاڵ دابنێ' : '2. First: Place stones on empty spots',
                        isKurdish ? '٣. دووەم: بەردەکان بجوڵێنە بۆ خانەی نزیک' : '3. Then: Slide stones to adjacent positions',
                        isKurdish ? '٤. ئامانج: ٣ بەرد لە ڕیزەکدا دابنێ!' : '4. Goal: Get 3 stones in a row!',
                    ].map((rule, i) => (
