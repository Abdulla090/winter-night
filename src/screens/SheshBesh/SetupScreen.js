...
import { Play, Dice5, Users, Info } from 'lucide-react-native';
...
export default function TawlaSetupScreen({ navigation }) {
    ...
    const handleStart = () => {
        ...
        navigation.navigate('SheshBeshPlay', {
    ...
    return (
        <AnimatedScreen>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
                ...
                <MotiView from={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
                    ...
                    <View style={[styles.heroIcon, { backgroundColor: colors.accent + '20' }]}>
                        <Dice5 size={48} color={colors.accent} strokeWidth={1.5} />
                    </View>
                    <Text style={[styles.heroSub, { color: colors.text.muted }, isKurdish && styles.kf]}>
                        {isKurdish ? 'تەولەی کوردی' : 'Kurdish Backgammon'}
                    </Text>
                </MotiView>
                ...
                <GlassCard style={{ marginBottom: layout.spacing.lg }}>
                    <View style={[styles.infoRow, { flexDirection: rowDirection }]}>
                        ...
                        <View style={styles.infoItem}>
                            <Dice5 size={16} color={colors.accent} />
                            <Text style={[styles.infoText, { color: colors.text.secondary }]}>2</Text>
                            <Text style={[styles.infoLabel, { color: colors.text.muted }, isKurdish && styles.kf]}>
                                {isKurdish ? 'تاس' : 'Dice'}
                            </Text>
                        ...
                        <View style={[styles.infoDivider, { backgroundColor: colors.border }]} />
                        <View style={styles.infoItem}>
                            <Text style={[styles.infoText, { color: colors.text.secondary }]}>15</Text>
                            <Text style={[styles.infoLabel, { color: colors.text.muted }, isKurdish && styles.kf]}>
                ...
                <GlassCard style={{ marginBottom: layout.spacing.lg }}>
                    <View style={[styles.sectionHeader, { flexDirection: rowDirection }]}>
                        <View style={[styles.playerDot, { backgroundColor: '#1C1C1C', borderColor: '#555' }]} />
                        <Text style={[styles.sectionTitle, { color: colors.text.secondary }, isKurdish && styles.kf]}>
                ...
                <GlassCard>
                    ...
                    {[
                        isKurdish ? '• تاس بهاوێژە و پاسەکانت بجوڵێنە' : '• Roll dice and move your pieces',
                        isKurdish ? '• پاسەکانت بەبەرەو ماڵەوەت بجوڵێنە' : '• Move pieces toward your home board',
                        isKurdish ? '• لەسەر پاسەی تاکی ڕکابەر دەتوانیت بیدەیت' : '• Hit single opponent pieces to the bar',
                        isKurdish ? '• تاسی دووانە = ٤ جوڵە!' : '• Doubles = 4 moves!',
                        isKurdish ? '• یەکەمین کەس هەموو ١٥ پاسەی دەربکات دەبات!' : '• First to bear off all 15 pieces wins!',
                    ].map((rule, i) => (
            ...
            <MotiView animate={{ translateY: 0, opacity: 1 }} style={styles.fabContainer}>
                <BeastButton
                    title={isKurdish ? 'تاس بهاوێژە!' : 'Roll the Dice!'}
                    onPress={handleStart}
                    variant="primary"
                    size="lg"
                    icon={Play}
